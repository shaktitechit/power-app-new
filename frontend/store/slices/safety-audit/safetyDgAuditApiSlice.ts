...
const base = "/v1/safety-dg-audits";
const tag = "SafetyDgAudit" as const;

export const safetyDgAuditApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getSafetyDgAudits: builder.query<ListSafetyAuditsResponse, SafetyAuditListArg>({
      query: (params) => ({
        url: base,
        method: "GET",
    ...
    getSafetyDgAuditById: builder.query<GetSafetyAuditByIdResponse, string>({
      query: (id) => ({ url: `${base}/${id}`, method: "GET" }),
      providesTags: (_r, _e, id) => [{ type: tag, id }],
    }),
    createSafetyDgAudit: builder.mutation<
      MutateSafetyAuditResponse,
      CreateSafetyAuditRequest
    >({
      query: (body) => ({
    ...
    updateSafetyDgAudit: builder.mutation<
      MutateSafetyAuditResponse,
      UpdateSafetyAuditRequest
    >({
      query: ({ id, ...body }) => ({
    ...
    deleteSafetyDgAudit: builder.mutation<DeleteSafetyAuditResponse, string>({
