...
const base = "/v1/safety-general-audits";
const tag = "SafetyGeneralAudit" as const;

export const safetyGeneralAuditApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getSafetyGeneralAudits: builder.query<ListSafetyAuditsResponse, SafetyAuditListArg>({
      query: (params) => ({
        url: base,
        method: "GET",
    ...
    getSafetyGeneralAuditById: builder.query<GetSafetyAuditByIdResponse, string>({
      query: (id) => ({ url: `${base}/${id}`, method: "GET" }),
      providesTags: (_r, _e, id) => [{ type: tag, id }],
    }),
    createSafetyGeneralAudit: builder.mutation<
      MutateSafetyAuditResponse,
      CreateSafetyAuditRequest
    >({
      query: (body) => ({
    ...
    updateSafetyGeneralAudit: builder.mutation<
      MutateSafetyAuditResponse,
      UpdateSafetyAuditRequest
    >({
      query: ({ id, ...body }) => ({
    ...
    deleteSafetyGeneralAudit: builder.mutation<DeleteSafetyAuditResponse, string>({
