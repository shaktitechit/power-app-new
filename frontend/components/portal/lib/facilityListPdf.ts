...
export type FacilityListPdfOrientation = "portrait" | "landscape";

export type FacilityListPdfInput = {
  rows: Facility[];
  ...
  brandName?: string;
  primaryColor?: string;
  orientation: FacilityListPdfOrientation;
  tabLabel?: string;
};

function layoutForOrientation(orientation: FacilityListPdfOrientation): PdfPageLayout {
...
function buildEqualColumnStyles(columnCount: number, contentW: number) {
  ...
  return Object.fromEntries(
    Array.from({ length: columnCount }, (_, index) => [index, { cellWidth }]),
  );
}

export function facilityListPdfFilename(
  orientation: FacilityListPdfOrientation,
  tab: string = "all",
) {
  const stamp = new Date().toISOString().split("T")[0];
  return `facilities_${tab}_${orientation}_${stamp}.pdf`;
}

export async function buildFacilityListPdfBlob(
  input: FacilityListPdfInput,
): Promise<Blob> {
  ...
  const tabSuffix = input.tabLabel ? ` • ${input.tabLabel}` : "";
  const compactSubtitle = `FACILITIES EXPORT${tabSuffix}    ${exportedOn}    ${input.rows.length} records`;
  ...
  doc.text("FACILITIES EXPORT", layout.pageW / 2, y, { align: "center" });
  y += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...MUTED);
  doc.text(
    `Exported on ${exportedOn}${tabSuffix}  •  ${input.rows.length} record${input.rows.length === 1 ? "" : "s"}`,
    layout.pageW / 2,
    y,
    { align: "center" },
  );
  ...
  const exportRows = buildFacilityExportRows(input.rows, input.columns);
