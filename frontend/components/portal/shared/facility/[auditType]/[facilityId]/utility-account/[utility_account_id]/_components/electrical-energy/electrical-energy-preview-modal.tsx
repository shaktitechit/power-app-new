...
function formatTabRowCount(tab: { rowCount: number; sections: { rows: { length: number }; columns: { key: string }[] }[] }) {
  if (tab.rowCount > 0) {
...
export function ElectricalEnergyPreviewModal({
  model,
  utilityAccount,
  disabled = false,
}: ElectricalEnergyPreviewModalProps) {
  ...
  const handleExportActiveTab = () => {
    ...
    exportPreviewTabToCsv(
      activePreviewTab,
      `utility-audit-${utilityAccount.account_number}-${safeLabel}`,
    );
  };
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  Review audit data with friendly column labels. Select a row to
                  mark records completed or pending. Equipment and audit data are
                ...
                return (
                  <section
                    ...
                    className={cn(
                      ...
                      activePreviewTab.sections.length > 1 ? "flex-1" : "flex-1",
                    )}
                  >
                    ...
                    <GoogleSheetGrid
                      ...
                      isMultiSelect={section.supportsCompletenessToggle && !sectionIsEmpty}
                      onSelectAll={() => handleSectionSelectAll(section)}
                      onRowSelect={
                        section.supportsCompletenessToggle && !sectionIsEmpty
                          ? (rowIndex) => handleSectionRowSelect(section.id, rowIndex)
                          : undefined
                      }
                    />
                  </section>
            ...
            {finalAuditLocked ? (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-muted-foreground">
                  This utility audit has been submitted. Open it again to mark all
                  included records as pending and continue editing.
                </p>
                ...
                {!canFinalSubmit ? (
                  <div className="mt-3 rounded-md border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                    <p className="font-medium">
                      Final submit is blocked until all required sections are completed.
                    </p>
                    <p className="mt-1">
