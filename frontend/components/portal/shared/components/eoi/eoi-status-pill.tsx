import { eoiStatusLabel } from "@/components/portal/lib/eoiConstants";

export function EoiStatusPill({ status }: { status: string }) {
  const base =
  ...
  return <span className={`${base} ${cls}`}>{eoiStatusLabel(status)}</span>;
}
