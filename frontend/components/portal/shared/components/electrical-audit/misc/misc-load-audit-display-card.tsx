...
import type { MiscLoadAuditRecord, MiscLoadAuditDocument } from "@/store/slices/electrical-audit/miscLoadAuditApiSlice";
...
import { auditToForm, formatDisplayValue, toDateInput } from "./misc-load-audit-utils";
...
type Props = {
  ...
  onPreviewDocument: (doc: MiscLoadAuditDocument, recordId: string, index: number) => void;
  onDeleteDocument?: (doc: MiscLoadAuditDocument, recordId: string, index: number) => void;
};

export function MiscLoadAuditDisplayCard({ record, tabLabel, auditStepLocked = false, canDelete = false, canViewDocuments = true, saving = false, onEdit, onDelete, onToggleCompleteness, togglingCompleteness = false, onUploadDocuments, onPreviewDocument, onDeleteDocument }: Props) {
    ...
    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-4">
      <Card className="min-w-0 overflow-hidden lg:col-span-3">
        ...
        <CardContent className="grid grid-cols-3 gap-x-3 gap-y-3 pb-4 pt-0">
          <DisplayField label="Equipment Name" value={form.equipment_name} />
          <DisplayField label="Category" value={form.category} />
          <DisplayField label="Location / Department" value={form.location_department} />
          <DisplayField label="Quantity" value={form.quantity} />
          <DisplayField label="Rated Power (kW)" value={form.rated_power_kW} />
          <DisplayField label="Avg Operating Hrs / Day" value={form.average_operating_hours_per_day} />
