import type { LightingAuditRecord, CreateLightingAuditRequest } from "@/store/slices/electrical-audit/lightingAuditApiSlice";
...
export type LightingAuditFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
...
export const computeLightingValues = (form: LightingAuditFormState) => {
...
export function buildLightingAuditPayload(
...
export function sortLightingAuditsStable(records: LightingAuditRecord[]): LightingAuditRecord[] {
...
export function getLightingAuditTabLabel(record: LightingAuditRecord, index: number) {
  const area = record.area_location?.trim();
  const prefix = `Lighting ${index + 1}`;
  return area ? `${prefix} (${area})` : prefix;
}

export function updateLightingAuditForm(
  form: LightingAuditFormState,
  updater: (form: LightingAuditFormState) => LightingAuditFormState,
): LightingAuditFormState {
  return computeLightingValues(updater(form));
}

export function applyLightingAuditExcelParsed(
  form: LightingAuditFormState,
  parsed: Record<string, unknown>,
): LightingAuditFormState {
  const next = { ...form } as LightingAuditFormState;
