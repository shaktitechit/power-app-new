...
import { FileText, ImageIcon, Pencil, Trash2, Upload, Cpu } from "lucide-react";
import {
  transformerToForm,
  formatDisplayValue,
} from "./transformer-utils";
import { TransformerAuditPanel } from "./transformer-audit-panel";
...
export function TransformerDisplayCard({
  ...
  const form = transformerToForm(transformer);
      ...
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-4">
        ...
        >
          ...
          <CardContent className="grid grid-cols-2 gap-x-3 gap-y-3 pb-4 pt-0 md:grid-cols-3">
            <DisplayField label="Rated Capacity (kVA)" value={form.rated_capacity_kVA} />
            <DisplayField label="Type of Cooling" value={form.type_of_cooling} />
            <DisplayField label="Rated HV (kV)" value={form.rated_HV_kV} />
            <DisplayField label="Rated LV (V)" value={form.rated_LV_V} />
            <DisplayField label="Rated HV Current (A)" value={form.rated_HV_current_A} />
            <DisplayField label="Rated LV Current (A)" value={form.rated_LV_current_A} />
            <DisplayField label="No Load Loss (kW)" value={form.no_load_loss_kW} />
            <DisplayField label="Full Load Loss (kW)" value={form.full_load_loss_kW} />
            <DisplayField label="Nameplate Efficiency (%)" value={form.nameplate_efficiency_percent} />
          </CardContent>
        </Card>
        ...
        <Card className="min-w-0 overflow-hidden lg:col-span-1">
          ...
          <CardContent className="min-w-0 space-y-3 pt-2">
            {!canViewDocuments ? (
            ...
            ) : (transformer.documents ?? []).length === 0 ? (
              ...
              <div className="grid min-w-0 gap-2">
                {(transformer.documents ?? []).map((doc, docIdx) => (
                  <div
                    key={docIdx}
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                      ...
                      <div className="flex min-w-0 flex-1 flex-col">
                        <button
                          type="button"
                          onClick={() =>
                            onPreviewDocument(doc, transformer._id, docIdx)
                          }
