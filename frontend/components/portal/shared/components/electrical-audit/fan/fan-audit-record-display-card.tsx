...
import type { FanAuditRecord, FanAuditDocument } from "@/store/slices/electrical-audit/fanAuditRecordApiSlice";
...
import { auditToForm, formatDisplayValue, toDateInput } from "./fan-audit-record-utils";
...
type Props = {
  ...
  onPreviewDocument: (doc: FanAuditDocument, recordId: string, index: number) => void;
  onDeleteDocument?: (doc: FanAuditDocument, recordId: string, index: number) => void;
};

export function FanAuditRecordDisplayCard({ record, tabLabel, auditStepLocked = false, canDelete = false, canViewDocuments = true, saving = false, onEdit, onDelete, onToggleCompleteness, togglingCompleteness = false, onUploadDocuments, onPreviewDocument, onDeleteDocument }: Props) {
    ...
    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-4">
      <Card className="min-w-0 overflow-hidden lg:col-span-3">
        ...
        <CardContent className="grid grid-cols-3 gap-x-3 gap-y-3 pb-4 pt-0">
          ...
          <DisplayField label="Fan Type" value={form.fan_type} />
          <DisplayField label="Make / Model" value={form.make_model} />
          <DisplayField label="Rated Power (W)" value={form.rated_power_W} />
          <DisplayField label="Measured Power (W)" value={form.measured_power_W} />
          <DisplayField label="Quantity (Nos)" value={form.quantity_nos} />
          <DisplayField label="Speed Control" value={form.speed_control_type} />
          <DisplayField label="Operating Hrs / Day" value={form.operating_hours_per_day} />
          <DisplayField label="Operating Days / Year" value={form.operating_days_per_year} />
          <DisplayField label="Loading Factor (%)" value={form.loading_factor_percent} />
          <DisplayField label="Connected Load (kW)" value={form.connected_load_kW} />
          <DisplayField label="Annual Energy (kWh)" value={form.annual_energy_consumption_kWh} />
