...
import { formatPanelTypeLabel } from "./panel-room-section";

interface SafetyPanelRoomDisplayCardProps {
...
export function SafetyPanelRoomDisplayCard({
  record,
  ...
  onUploadDocuments,
  onPreviewDocument,
}: SafetyPanelRoomDisplayCardProps) {
  const panelName =
    typeof record.panel_name === "string" && record.panel_name.trim()
      ? record.panel_name
      : "Panel Room Audit Record";

  return (
    <Card className="border-border bg-card">
      <CardHeader className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 pb-4">
        <CardTitle className="text-base font-semibold">{panelName}</CardTitle>
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">
              {formatPanelTypeLabel(record.panel_type)}
            </p>
          </div>
          <div>
