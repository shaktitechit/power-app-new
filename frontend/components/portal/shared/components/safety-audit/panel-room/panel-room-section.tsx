...
import { SafetyPanelRoomDisplayCard } from "./safety-panel-room-display-card";
import { SafetyPanelRoomFormModal } from "./safety-panel-room-form-modal";
...
export const PANEL_TYPES = ["pdb", "mdb", "sub_panel", "other"] as const;
export type PanelType = (typeof PANEL_TYPES)[number];

export const PANEL_TYPE_OPTIONS: { value: PanelType; label: string }[] = [
  { value: "pdb", label: "PDB" },
  { value: "mdb", label: "MDB" },
...
export function normalizePanelType(v: unknown): PanelType {
  const s = typeof v === "string" ? v : "";
  return PANEL_TYPES.includes(s as PanelType) ? (s as PanelType) : "pdb";
}

export function formatPanelTypeLabel(v: unknown): string {
  const normalized = normalizePanelType(v);
  return PANEL_TYPE_OPTIONS.find((o) => o.value === normalized)?.label ?? "-";
}
...
export type SafetyPanelRoomFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
const DEFAULT_PANEL_ROOM_CHECKLIST_SPEC: readonly {
  ...
  {
    activity_description: "Verify panel identification of PDB.",
    requirement: "Title/nameplate clearly visible and legible.",
  },
  {
    activity_description: "Check load monitoring provision at PDB.",
    requirement: "Ammeter or energy meter installed and functional.",
  },
  {
    activity_description: "Inspect padlocking provision for feeders (LOTO).",
    requirement: "Lockout/Tagout (LOTO) provision available and operable.",
  },
  {
    activity_description: "Verify feeders are provided with protective devices.",
    requirement: "Adequate overload and short-circuit protection installed.",
  },
  {
    activity_description: "Check isolation provision for control circuit.",
    requirement: "MCB or equivalent isolation device provided.",
  },
  {
    activity_description: "Inspect earthing provision for panel room equipment.",
    requirement: "Two-point earthing provided and securely connected.",
  },
  {
    activity_description: "Check panel door locking mechanism.",
    requirement: "Door lock functional and ensures restricted access.",
  },
];
...
function recordToForm(r: SafetyAuditRecord): SafetyPanelRoomFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    panel_name: typeof r.panel_name === "string" ? r.panel_name : "",
    panel_type: normalizePanelType(r.panel_type),
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  if (typeof record.panel_name === "string" && record.panel_name.trim()) {
    return record.panel_name.trim();
  }
  return `Panel Room ${index + 1}`;
}
...
export function SafetyPanelRoomSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyPanelRoomAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyPanelRoomFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyPanelRoomFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      panel_name: modalForm.panel_name.trim() || undefined,
      panel_type: modalForm.panel_type,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating panel room safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety panel room audits…
      </div>
    );
  }
  ...
  return (
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between panel room safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      <SafetyPanelRoomFormModal
        ...
        panelTypeOptions={PANEL_TYPE_OPTIONS}
      />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this panel room safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("panel-room-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="panel-room-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
