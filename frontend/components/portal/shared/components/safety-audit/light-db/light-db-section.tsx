...
import { SafetyLightDbDisplayCard } from "./safety-light-db-display-card";
import { SafetyLightDbFormModal } from "./safety-light-db-form-modal";
...
export type SafetyLightDbFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(r: SafetyAuditRecord): SafetyLightDbFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    ldb_name: typeof r.ldb_name === "string" ? r.ldb_name : "",
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  if (typeof record.ldb_name === "string" && record.ldb_name.trim()) {
    return record.ldb_name.trim();
  }
  return `Light DB ${index + 1}`;
}
...
export function SafetyLightDbSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyLdbAuditsQuery({
    utility_account_id: utilityAccountId,
  });

  const [createRec, { isLoading: isCreating }] = useCreateSafetyLdbAuditMutation();
  const [updateRec, { isLoading: isUpdating }] = useUpdateSafetyLdbAuditMutation();
  const [deleteRec, { isLoading: isDeleting }] = useDeleteSafetyLdbAuditMutation();
  ...
  const [modalForm, setModalForm] = useState<SafetyLightDbFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyLightDbFormState | null>(null);
  ...
  const activeRecord = useMemo(
    () => sortedRecords.find((r) => r._id === activeTabId) ?? null,
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      ldb_name: modalForm.ldb_name.trim() || undefined,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating light DB safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety light DB audits…
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between light DB safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this light DB safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("light-db-record-doc-input")?.click()
              }
            >
