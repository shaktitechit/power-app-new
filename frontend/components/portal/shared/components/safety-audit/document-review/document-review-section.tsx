...
import { SafetyDocumentReviewDisplayCard } from "./safety-document-review-display-card";
import { SafetyDocumentReviewFormModal } from "./safety-document-review-form-modal";
...
export type SafetyDocumentReviewFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
const DEFAULT_DOCUMENTS_REVIEW_CHECKLIST_SPEC: readonly {
  ...
  },
  {
    ...
    requirement: "Chart in Hindi + local language, laminated, visible",
  },
  {
    activity_description: "Chart content accuracy",
    requirement: "Includes first aid, CPR steps, emergency contacts",
  },
  {
    activity_description: "Training records available",
    requirement: "Electrical safety & first aid logs maintained annually",
  },
  {
  ...
  },
  {
    ...
    requirement: "Separation from electrical earth confirmed",
  },
  {
  ...
  {
    activity_description: "Check for cable and equipment IR test reports",
    requirement: "Last 3 test data",
  },
  {
    activity_description: "AMC for major electrical equipment",
    requirement: "DG/ Compressor/ elevators/UPS/Batteries.",
  },
  {
    activity_description:
      "Transformer oil test records available (Break Down Voltage- BDV and Dissolved Gas Analysis-DGA)",
    requirement: "Last 3-year test reports (to be carried out at least once in a year)",
  },
  {
  ...
  {
    activity_description: "Records of electrical safety training",
  ...
  {
    activity_description: "Calibration certificates for electrical measuring instruments",
    requirement: "Check last 3-year reports",
  },
  {
    activity_description:
...
const createEmptyForm = (): SafetyDocumentReviewFormState => ({
...
function recordToForm(r: SafetyAuditRecord): SafetyDocumentReviewFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    equipment_name: typeof r.equipment_name === "string" ? r.equipment_name : "",
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  if (typeof record.equipment_name === "string" && record.equipment_name.trim()) {
    return record.equipment_name.trim();
  }
  return `Document Review ${index + 1}`;
}
...
export function SafetyDocumentsReviewSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyDocumentsAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyDocumentReviewFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyDocumentReviewFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      equipment_name: modalForm.equipment_name.trim() || undefined,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating documents review safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety documents review audits…
      </div>
    );
  }
    ...
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between document review safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      {sortedRecords.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {activeRecord ? (
            <SafetyDocumentReviewDisplayCard
              record={activeRecord}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this documents review safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("document-review-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="document-review-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
