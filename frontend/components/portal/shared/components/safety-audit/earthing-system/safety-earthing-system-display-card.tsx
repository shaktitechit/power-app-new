...
import { formatPitTypeLabel } from "./earthing-system-section";

interface SafetyEarthingSystemDisplayCardProps {
...
export function SafetyEarthingSystemDisplayCard({
  record,
  ...
  onUploadDocuments,
  onPreviewDocument,
}: SafetyEarthingSystemDisplayCardProps) {
  const title =
    typeof record.pit_name === "string" && record.pit_name.trim()
      ? record.pit_name.trim()
      : "Earthing System Audit Record";
    ...
    <Card className="border-border bg-card">
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          <div>
            <p className="text-xs text-muted-foreground">Pit type</p>
            <p className="mt-1 text-sm font-medium text-foreground">
              {formatPitTypeLabel(record.pit_type)}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Resistance (Ω)</p>
            <p className="mt-1 text-sm font-medium text-foreground">
              {record.resistance_ohm != null ? String(record.resistance_ohm) : "-"}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Test date</p>
            <p className="mt-1 text-sm font-medium text-foreground">
              {typeof record.test_date === "string" && record.test_date
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">
              {typeof record.due_date === "string" && record.due_date
