...
import { SafetyPumpCompressorDisplayCard } from "./safety-pump-compressor-display-card";
import { SafetyPumpCompressorFormModal } from "./safety-pump-compressor-form-modal";
...
export const EQUIPMENT_TYPES = ["pump", "compressor", "motor", "other"] as const;
...
export function normalizeEquipmentType(v: unknown): EquipmentType {
  const s = typeof v === "string" ? v : "";
  return EQUIPMENT_TYPES.includes(s as EquipmentType) ? (s as EquipmentType) : "pump";
}

export function formatEquipmentTypeLabel(v: unknown): string {
  const normalized = normalizeEquipmentType(v);
  return EQUIPMENT_TYPE_OPTIONS.find((o) => o.value === normalized)?.label ?? "-";
}
...
export type SafetyPumpCompressorFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(r: SafetyAuditRecord): SafetyPumpCompressorFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    equipment_type: normalizeEquipmentType(r.equipment_type),
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  ...
  return `Pump/Compressor ${index + 1}`;
}
...
export function SafetyPumpCompressorSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyPumpCompressorAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyPumpCompressorFormState | null>(
    null,
  );

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyPumpCompressorFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      equipment_type: modalForm.equipment_type,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating pump & compressor safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading pump & compressor safety audits…
      </div>
    );
  }
    ...
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between pump & compressor safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      {sortedRecords.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {activeRecord ? (
            <SafetyPumpCompressorDisplayCard
              record={activeRecord}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this pump & compressor safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("pump-compressor-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="pump-compressor-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
