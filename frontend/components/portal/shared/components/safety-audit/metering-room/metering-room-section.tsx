...
import { SafetyMeteringRoomDisplayCard } from "./safety-metering-room-display-card";
import { SafetyMeteringRoomFormModal } from "./safety-metering-room-form-modal";
...
export type SafetyMeteringRoomFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
const DEFAULT_METERING_ROOM_CHECKLIST_SPEC: readonly {
  ...
  {
    activity_description: "Check the incomers are correctly identified",
  ...
  {
    activity_description: "Check protection provision in incomer MCCB",
...
function recordToForm(r: SafetyAuditRecord): SafetyMeteringRoomFormState {
  const items =
...
export function SafetyMeteringRoomSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyMeteringRoomAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyMeteringRoomFormState | null>(
    null,
  );

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyMeteringRoomFormState | null>(
    null,
  );
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating metering room safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety metering room audits…
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between metering room safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this metering room safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("metering-room-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="metering-room-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
