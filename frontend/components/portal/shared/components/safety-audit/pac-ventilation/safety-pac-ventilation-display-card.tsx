...
import { formatUnitTypeLabel } from "./pac-ventilation-section";

interface SafetyPacVentilationDisplayCardProps {
...
export function SafetyPacVentilationDisplayCard({
  record,
  ...
  onUploadDocuments,
  onPreviewDocument,
}: SafetyPacVentilationDisplayCardProps) {
  const title =
    typeof record.unit_name === "string" && record.unit_name.trim()
      ? record.unit_name.trim()
      : "PAC & Ventilation Audit Record";
    ...
    <Card className="border-border bg-card">
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            <p className="text-xs text-muted-foreground">Room temperature (°C)</p>
            <p className="mt-1 text-sm font-medium text-foreground">
              {record.room_temperature_c != null
                ? String(record.room_temperature_c)
                : "-"}
            </p>
          </div>
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">
              {record.relative_humidity_percent != null
                ? String(record.relative_humidity_percent)
                : "-"}
            </p>
          </div>
          <div>
