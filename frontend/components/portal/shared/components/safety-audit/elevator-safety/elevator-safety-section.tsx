...
import { SafetyElevatorSafetyDisplayCard } from "./safety-elevator-safety-display-card";
import { SafetyElevatorSafetyFormModal } from "./safety-elevator-safety-form-modal";
...
export type SafetyElevatorSafetyFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(r: SafetyAuditRecord): SafetyElevatorSafetyFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    elevator_name: typeof r.elevator_name === "string" ? r.elevator_name : "",
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  if (typeof record.elevator_name === "string" && record.elevator_name.trim()) {
    return record.elevator_name.trim();
  }
  return `Elevator ${index + 1}`;
}
...
export function SafetyElevatorSafetySection({
  ...
  const { data, isLoading, refetch } = useGetSafetyElevatorAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyElevatorSafetyFormState | null>(
    null,
  );

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyElevatorSafetyFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      elevator_name: modalForm.elevator_name.trim() || undefined,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating elevator safety audit..."
          : "Saving...",
    ...
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between elevator safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      {sortedRecords.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {activeRecord ? (
            <SafetyElevatorSafetyDisplayCard
              record={activeRecord}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this elevator safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("elevator-safety-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="elevator-safety-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
