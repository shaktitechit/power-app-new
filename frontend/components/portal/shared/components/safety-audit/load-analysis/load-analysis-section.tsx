...
import { SafetyLoadAnalysisDisplayCard } from "./safety-load-analysis-display-card";
import { SafetyLoadAnalysisFormModal } from "./safety-load-analysis-form-modal";
...
export type SafetyLoadAnalysisFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
const toPercentString = (value: unknown): string => {
...
function recordToForm(r: SafetyAuditRecord): SafetyLoadAnalysisFormState {
  const items =
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    transformer_loading_percent: toPercentString(r.transformer_loading_percent),
    panel_breaker_loading_percent: toPercentString(r.panel_breaker_loading_percent),
    current_unbalance_percent: toPercentString(r.current_unbalance_percent),
    voltage_unbalance_percent: toPercentString(r.voltage_unbalance_percent),
...
export function SafetyLoadAnalysisSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyLoadAnalysisAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyLoadAnalysisFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyLoadAnalysisFormState | null>(
    null,
  );
  ...
  const parsePercentField = (val: string): number | undefined => {
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      transformer_loading_percent: parsePercentField(
        modalForm.transformer_loading_percent,
      ),
      panel_breaker_loading_percent: parsePercentField(
        modalForm.panel_breaker_loading_percent,
      ),
      current_unbalance_percent: parsePercentField(modalForm.current_unbalance_percent),
      voltage_unbalance_percent: parsePercentField(modalForm.voltage_unbalance_percent),
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating load analysis safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety load analysis audits…
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between load analysis safety records using the tabs. Add or edit
            details via modals.
          </p>
        </div>
        {!auditStepLocked && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this load analysis safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("load-analysis-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="load-analysis-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
