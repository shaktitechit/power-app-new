...
import { SafetyThermographyDisplayCard } from "./safety-thermography-display-card";
import { SafetyThermographyFormModal } from "./safety-thermography-form-modal";
...
export type SafetyThermographyFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function defaultThermographyChecklistRows(): ChecklistRow[] {
...
const createEmptyForm = (): SafetyThermographyFormState => ({
  ...
  status: "draft",
  items: defaultThermographyChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(r: SafetyAuditRecord): SafetyThermographyFormState {
  const items =
    Array.isArray(r.items) && r.items.length > 0
      ? (r.items as SafetyChecklistItem[]).map((it, i) => itemToRow(it, i))
      : defaultThermographyChecklistRows();
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    inspection_date: toDateInput(r.inspection_date as string | undefined),
    inspected_by: typeof r.inspected_by === "string" ? r.inspected_by : "",
    location: typeof r.location === "string" ? r.location : "",
    max_temperature_c: r.max_temperature_c != null ? String(r.max_temperature_c) : "",
    delta_temperature_c:
      r.delta_temperature_c != null ? String(r.delta_temperature_c) : "",
...
export function SafetyThermographySection({
  ...
  const { data, isLoading, refetch } = useGetSafetyThermographyAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyThermographyFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyThermographyFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      inspection_date: modalForm.inspection_date || undefined,
      inspected_by: modalForm.inspected_by.trim() || undefined,
      location: modalForm.location.trim() || undefined,
      max_temperature_c: toNumber(modalForm.max_temperature_c),
      delta_temperature_c: toNumber(modalForm.delta_temperature_c),
      urgency: modalForm.urgency.trim() || undefined,
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating thermography safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading safety thermography audits…
      </div>
    );
  }
