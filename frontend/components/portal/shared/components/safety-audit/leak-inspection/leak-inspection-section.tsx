...
import { SafetyLeakInspectionDisplayCard } from "./safety-leak-inspection-display-card";
import { SafetyLeakInspectionFormModal } from "./safety-leak-inspection-form-modal";
...
export function formatLeakageFound(v: unknown): string {
  return v ? "Yes" : "No";
}
...
export type SafetyLeakInspectionFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(r: SafetyAuditRecord): SafetyLeakInspectionFormState {
  const items =
  ...
  return {
    ...
    equipment_name:
      typeof r.equipment_name === "string" ? r.equipment_name : "",
    location: typeof r.location === "string" ? r.location : "",
    leakage_found: r.leakage_found ? "true" : "false",
...
function recordTabLabel(record: SafetyAuditRecord, index: number): string {
  const name =
    typeof record.equipment_name === "string" ? record.equipment_name.trim() : "";
  if (name) return name;
  const eq =
    typeof record.equipment_type === "string" ? record.equipment_type : "";
  const label = EQUIPMENT_TYPE_OPTIONS.find((o) => o.value === eq)?.label ?? eq;
  return label ? `${label} ${index + 1}` : `Leak Inspection ${index + 1}`;
}
...
export function SafetyLeakInspectionSection({
  ...
  const { data, isLoading, refetch } = useGetSafetyLeakInspectionAuditsQuery({
  ...
  const [modalForm, setModalForm] = useState<SafetyLeakInspectionFormState | null>(
    null,
  );
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      leakage_found: modalForm.leakage_found === "true",
    ...
    try {
      const result = await toastHandler({
        ...
        loading: modalForm.isNew
          ? "Creating leak inspection safety audit..."
          : "Saving...",
  ...
  if (isLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading leak inspection safety audits…
      </div>
    );
  }
    ...
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Switch between leak inspection safety records using the tabs. Add or
      ...
      {sortedRecords.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {activeRecord ? (
            <SafetyLeakInspectionDisplayCard
              record={activeRecord}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this leak inspection safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() =>
                document.getElementById("leak-inspection-record-doc-input")?.click()
              }
            >
              ...
              <input
                id="leak-inspection-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
