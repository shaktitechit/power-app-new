...
interface SafetyLeakInspectionDisplayCardProps {
...
export function SafetyLeakInspectionDisplayCard({
  record,
  ...
  onUploadDocuments,
  onPreviewDocument,
}: SafetyLeakInspectionDisplayCardProps) {
  const title =
    typeof record.equipment_name === "string" && record.equipment_name.trim()
      ? record.equipment_name.trim()
      : "Leak Inspection Audit Record";
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">
              {formatLeakageFound(record.leakage_found)}
            </p>
          </div>
