...
import type { SafetyAuditRecord } from "@/store/slices/safety-audit/safetyAuditTypes";

interface SafetyTransformerDisplayCardProps {
  ...
  onPreviewDocument: (doc: any, index: number) => void;
  transformers: any[];
}

export function SafetyTransformerDisplayCard({
  record,
  ...
  onPreviewDocument,
  transformers,
}: SafetyTransformerDisplayCardProps) {
  const linkedTransformerId = typeof record.transformer_id === "object" && record.transformer_id
    ? (record.transformer_id as any)._id
    : record.transformer_id;

  const linkedTransformer = transformers.find((t) => t._id === linkedTransformerId);
  const transformerLabel = linkedTransformer?.transformer_tag ||
    (typeof record.transformer_id === "object" && record.transformer_id ? (record.transformer_id as any).name : null) ||
    "-";
    ...
    <Card className="border-border bg-card">
      <CardHeader className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 pb-4">
        <CardTitle className="text-base font-semibold">
          {record.name || "Transformer Audit Record"}
        </CardTitle>
        <div className="flex flex-wrap items-center justify-end gap-2">
          {/* Completeness toggle: always shown when marking completed, but
              marking pending is restricted to privileged roles only */}
          ...
          <div className="flex items-center gap-2">
            {/* Edit and Delete are hidden when record is completed */}
            ...
            {canDelete && !record.is_completed && (
              <Button variant="destructive" size="sm" onClick={onDelete} disabled={saving || auditStepLocked}>
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">{record.location || "-"}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Capacity (kVA)</p>
            <p className="mt-1 text-sm font-medium text-foreground">{record.capacity_kva || "-"}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Linked Transformer</p>
            <p className="mt-1 text-sm font-medium text-foreground">{transformerLabel}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Audit Date</p>
            <p className="mt-1 text-sm font-medium text-foreground">
              {record.audit_date ? new Date(record.audit_date).toLocaleDateString() : "-"}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Status</p>
            <p className="mt-1 text-sm font-medium capitalize text-foreground">{record.status || "draft"}</p>
          </div>
        </div>
              ...
              <tbody>
                {record.items?.map((item, idx) => (
                  <tr key={idx} className="border-b last:border-0 hover:bg-muted/10">
                    ...
                    <td className="p-2 text-foreground font-medium">{item.activity_description}</td>
                    ...
                    <td className="p-2 text-muted-foreground">{item.recommendations || "-"}</td>
            ...
            record.documents && record.documents.length > 0 ? (
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {record.documents.map((doc, idx) => (
                  <div key={idx} className="flex items-start gap-3 rounded-lg border p-2 bg-background">
                    {doc.fileType === "image" ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {doc.caption ? (
                        <p className="truncate text-xs text-muted-foreground" title={doc.caption}>
                          {doc.caption}
                        </p>
                      ) : null}
                      {doc.uploadedAt ? (
