...
import type { SafetyTransformerFormState, ChecklistRow } from "./transformer-section";
...
type Props = {
  ...
  form: SafetyTransformerFormState | null;
  onFormChange: (
    updater: (prev: SafetyTransformerFormState) => SafetyTransformerFormState,
  ) => void;
  onSave: () => void;
  saving?: boolean;
  transformers: any[];
...
export function SafetyTransformerFormModal({
  open,
  onOpenChange,
  form,
  onFormChange,
  ...
  const inputClass = "border-input bg-background text-foreground focus:border-primary focus:ring-1 focus:ring-primary";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>
            {form.isNew ? "Add Transformer Safety Audit" : "Edit Transformer Safety Audit"}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  onFormChange((f) => ({ ...f, capacity_kva: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              >
                ...
                !transformers.some((t) => t._id === form.transformer_id) ? (
                  <option value={form.transformer_id}>
                    {form.transformerLabel || `Linked transformer (${form.transformer_id})`}
                  </option>
                ) : null}
                {transformers.map((t) => (
                ...
                <tbody>
                  {form.items.map((row) => (
                    <tr key={row.localKey} className="border-b last:border-0">
                      ...
                      <td className="p-1">
                        <Textarea className={frozenFieldClass} rows={2} value={row.activity_description} readOnly />
                      </td>
                      <td className="p-1">
                        <Textarea className={frozenFieldClass} rows={2} value={row.requirement} readOnly />
                      </td>
                      <td className="p-1">
                        <select
                          ...
                          onChange={(e) =>
                            updateRow(row.localKey, { compliance: e.target.value as any })
                          }
                        >
                      ...
                      <td className="p-1">
                        <Textarea
                          ...
                          rows={2}
                          value={row.remarks}
                          onChange={(e) => updateRow(row.localKey, { remarks: e.target.value })}
                        />
                      </td>
                      <td className="p-1">
                        <Textarea
                          ...
                          onChange={(e) => updateRow(row.localKey, { recommendations: e.target.value })}
                        />
                      </td>
                      <td className="p-1">
                        <select
                          ...
                          value={row.severity || ""}
                          onChange={(e) =>
                            updateRow(row.localKey, { severity: e.target.value as any })
                          }
                        >
