...
import { canViewDocuments, canUncompleteUtilityAuditStep, type UserPermission,
...
import { SafetyTransformerDisplayCard } from "./safety-transformer-display-card";
import { SafetyTransformerFormModal } from "./safety-transformer-form-modal";
...
export type SafetyTransformerFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(r: SafetyAuditRecord): SafetyTransformerFormState {
  ...
  return {
    ...
    name: r.name || "",
    location: r.location || "",
    capacity_kva: r.capacity_kva != null ? String(r.capacity_kva) : "",
...
export function SafetyTransformerSection({
  ...
  const { data: transformersData } = useGetTransformersQuery({ facility_id: facilityId });
  const transformers = useMemo(() => transformersData?.data || [], [transformersData]);

  const { data, isLoading, refetch } = useGetSafetyTransformerAuditsQuery({
    utility_account_id: utilityAccountId,
  });

  const [createRec, { isLoading: isCreating }] = useCreateSafetyTransformerAuditMutation();
  const [updateRec, { isLoading: isUpdating }] = useUpdateSafetyTransformerAuditMutation();
  const [deleteRec, { isLoading: isDeleting }] = useDeleteSafetyTransformerAuditMutation();
  ...
  const [modalForm, setModalForm] = useState<SafetyTransformerFormState | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SafetyTransformerFormState | null>(null);
  ...
  const recordTabs = useMemo(
    () =>
      sortedRecords.map((record, index) => ({
        id: record._id,
        label: record.name || `Transformer ${index + 1}`,
      })),
    [sortedRecords],
  );
  ...
  const [togglingCompletenessId, setTogglingCompletenessId] = useState<string | null>(null);

  const handleToggleCompleteness = async (rec: SafetyAuditRecord) => {
    const nextVal = !rec.is_completed;
    // Only admin/super_admin/manager can mark pending (uncomplete)
    if (rec.is_completed && !canMarkPending) {
      toast.error("You don't have permission to mark this record as pending.");
      return;
    }
    setTogglingCompletenessId(rec._id);
    try {
      await toastHandler({
        action: () => updateRec({ id: rec._id, is_completed: nextVal } as any).unwrap(),
        loading: "Updating status...",
        success: nextVal ? "Marked as completed" : "Marked as pending",
      });
      void refetch();
    } catch (e) {
      console.error(e);
    } finally {
      setTogglingCompletenessId(null);
    }
  };
  ...
  const handleSaveModal = async () => {
    ...
    const payload = {
      ...
      utility_account_id: utilityAccountId,
      name: modalForm.name.trim() || undefined,
      location: modalForm.location.trim() || undefined,
      capacity_kva: toNumber(modalForm.capacity_kva),
      transformer_id: modalForm.transformer_id.trim() || undefined,
      audit_date: modalForm.audit_date || undefined,
      status: (modalForm.status || "draft") as any,
    ...
    try {
      const result = await toastHandler({
        action: () => {
          if (modalForm.isNew) {
            return createRec(payload as any).unwrap();
          } else {
            return updateRec({ id: modalForm.id, ...payload } as any).unwrap();
          }
        },
        loading: modalForm.isNew ? "Creating transformer safety audit..." : "Saving...",
  ...
  const handleUploadDocs = async () => {
    ...
    try {
      await toastHandler({
        action: () => updateRec({ id: uploadModalRecordId, ...payload } as any).unwrap(),
  ...
  const {
    ...
    getDocuments: (recordId) => records.find((r) => r._id === recordId)?.documents,
    persist: (recordId, remaining) =>
      updateRec({ id: recordId, existing_documents: remaining } as never).unwrap(),
  });

  if (isLoading) {
    return <div className="text-sm text-muted-foreground">Loading safety transformer audits…</div>;
  }
    ...
    <div className="relative space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <h3 className="text-lg font-semibold text-foreground">Safety — Transformer</h3>
          <p className="text-sm text-muted-foreground">
            Switch between transformer safety records using the tabs. Add or edit details via modals.
          </p>
        </div>
        {!auditStepLocked && (
          <Button
        ...
        <div className="space-y-4">
          ...
          {activeRecord && activeRecordIndex >= 0 ? (
            <SafetyTransformerDisplayCard
              ...
              onToggleCompleteness={() => void handleToggleCompleteness(activeRecord)}
              togglingCompleteness={togglingCompletenessId === activeRecord._id}
              onUploadDocuments={() => handleOpenUploadModal(activeRecord._id)}
              onPreviewDocument={handleOpenPreview}
              onDeleteDocument={(doc, index) =>
                requestDeleteDocument(activeRecord._id, index, doc.fileName)
              }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this transformer safety audit record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="space-y-4 py-2">
            <div
              ...
              onClick={() => document.getElementById("safety-record-doc-input")?.click()}
            >
              <Upload className="mb-2 h-8 w-8 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">Click to select files (PDF, images)</p>
              <input
                id="safety-record-doc-input"
                type="file"
                multiple
                accept="image/*,.pdf"
                className="hidden"
            ...
            {uploadFiles.length > 0 && (
              <ul className="space-y-3">
                {uploadFiles.map((item) => (
                  <li key={item.id} className="space-y-2 rounded-md border p-3 bg-muted/20">
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate text-sm font-medium">{item.file.name}</span>
                      <button
                        type="button"
                        onClick={() => setUploadFiles((prev) => prev.filter((entry) => entry.id !== item.id))}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="button" onClick={handleUploadDocs} disabled={uploadFiles.length === 0 || isUpdating}>
              Upload
            </Button>
          </div>
        </DialogContent>
