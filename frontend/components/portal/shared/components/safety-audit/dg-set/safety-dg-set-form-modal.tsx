...
import type { ChecklistRow, SafetyDgSetFormState } from "./dg-set-section";
...
type Props = {
  ...
  form: SafetyDgSetFormState | null;
  onFormChange: (updater: (prev: SafetyDgSetFormState) => SafetyDgSetFormState) => void;
  onSave: () => void;
  saving?: boolean;
  dgSets: DgSetOption[];
};

export function SafetyDgSetFormModal({
  open,
  onOpenChange,
  form,
  onFormChange,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>
            {form.isNew ? "Add DG Set Safety Audit" : "Edit DG Set Safety Audit"}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <Label>Linked DG set</Label>
              <select
                className={nativeSelectClassForm(false)}
                value={form.dg_set_id}
                onChange={(e) => {
                  const id = e.target.value;
                  const inList = Boolean(id && dgSets.some((d) => d._id === id));
                  onFormChange((f) => ({
                    ...f,
                    dg_set_id: id,
              ...
              >
                ...
                {form.dg_set_id && !dgSets.some((d) => d._id === form.dg_set_id) ? (
                  <option value={form.dg_set_id}>
                    {form.dgSetLabel || `Linked DG set (${form.dg_set_id})`}
                  </option>
                ) : null}
                {dgSets.map((dg) => (
                  <option key={dg._id} value={dg._id}>
                    {dg.dg_number?.trim() || dg.name?.trim() || dg._id}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onFormChange((f) => ({
                    ...f,
                    status: e.target.value as SafetyDgSetFormState["status"],
                  }))
                }
              >
                    ...
                    <tr key={row.localKey} className="border-b last:border-0">
                      ...
                      <td className="p-1">
                        <Textarea
                          ...
                          onChange={(e) =>
                            updateRow(row.localKey, { recommendations: e.target.value })
                          }
                        />
                      </td>
                      <td className="p-1">
