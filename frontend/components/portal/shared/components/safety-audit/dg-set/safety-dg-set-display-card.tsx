...
type DgSetOption = { _id: string; dg_number?: string; name?: string };

interface SafetyDgSetDisplayCardProps {
...
function resolveLinkedDgLabel(record: SafetyAuditRecord, dgSets: DgSetOption[]): string {
  const ref = record.dg_set_id;
  if (ref && typeof ref === "object") {
    const dg = ref as { dg_number?: string; name?: string };
    return dg.dg_number?.trim() || dg.name?.trim() || "-";
  }
  const id = typeof ref === "string" ? ref : "";
  if (id) {
    const found = dgSets.find((d) => d._id === id);
    if (found) return found.dg_number?.trim() || found.name?.trim() || id;
  }
  return "-";
}

export function SafetyDgSetDisplayCard({
  record,
  ...
  onPreviewDocument,
  dgSets,
}: SafetyDgSetDisplayCardProps) {
  const linkedDgLabel = resolveLinkedDgLabel(record, dgSets);
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">
              {record.capacity_kva != null ? String(record.capacity_kva) : "-"}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Linked DG</p>
            <p className="mt-1 text-sm font-medium text-foreground">{linkedDgLabel}</p>
          </div>
          <div>
