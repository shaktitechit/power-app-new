...
const { LightingAuditRecord } = modelsRegistry;
...
export const createLightingAuditRecordService = async ({ user, body, files, reqQuery, id, params }) => {
  ...
  const docs = await uploadAuditDocuments(files || [], "lighting-audits", recordId);
...
export const getLightingAuditRecordsService = async ({ user, body, files, reqQuery, id, params }) => {
...
export const getLightingAuditRecordByIdService = async ({ user, body, files, reqQuery, id, params }) => {
  const record = await LightingAuditRecord.findById(id)
    .populate("facility_id", "name city")
    .populate("utility_account_id", "account_number")
    .populate("auditor_id", "name email");
...
export const updateLightingAuditRecordService = async ({ user, body, files, reqQuery, id, params }) => {
  const record = await LightingAuditRecord.findById(id);
  ...
  const docs = await uploadAuditDocuments(files || [], "lighting-audits", record._id);
...
export const uploadLightingAuditDocumentsService = async ({
  user,
  id,
  files,
  body,
...
export const deleteLightingAuditRecordService = async ({ user, body, files, reqQuery, id, params }) => {
  const record = await LightingAuditRecord.findById(id);
